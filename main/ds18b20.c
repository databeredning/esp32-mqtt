...
/*
tempLSB = ds2482_1wire_read_byte();
tempMSB = ds2482_1wire_read_byte();
raw_temp = 10 * (uint16_t)((uint16_t)( tempMSB << 8 ) + (uint16_t)tempLSB );
if( adr[0] == 0x10 )
  new_temp = raw_temp >> 1;
else if( adr[0] == 0x28 )
  new_temp = raw_temp >> 4;
new_temp *= 10; // format CCc0 ie. CC.c 1/10 degrees Celsius
*/
...
//        temp=(float)(temp1+(temp2*256))/2;  // ds1820
        return temp;
      }
      else{return 0;}
